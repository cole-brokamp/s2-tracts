...
Every released file uses the `census-tracts-YEAR-r1` FlatGeobuf header title.
...
Run `Rscript scripts/assets.R freeze` after all years are prepared to compress and pin the exact release assets before building the binaries.
Use `Rscript scripts/assets.R freeze --replace` only when intentionally replacing pinned assets, then rebuild the binaries with the new manifest.
